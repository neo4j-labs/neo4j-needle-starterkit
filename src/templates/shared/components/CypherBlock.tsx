...
export default function CypherBlock(props: CypherBlockProps) {
  ...
  function setDriverFromProps() {
    ...
    if (props.neo4jConnection && typeof props.neo4jConnection === 'string') {
      ...
      return setDriver(connectionDetails.uri, connectionDetails.user, connectionDetails.password).then(
        (d: Driver | boolean) => {
  ...
  function submitQuery(query: string) {
    ...
    !queryLimit
      ? toast.danger(`You have not set a LIMIT in your return. We have applied a default of ${props.limitResultSet}`, {
          isCloseable: true,
          shouldAutoClose: true,
        })
      : null;
    isMultiQuery
      ? toast.danger('Multi-query detected, only single queries are supported. Execution has been cancelled.', {
          isCloseable: true,
          shouldAutoClose: true,
        })
      : null;
        ...
        >
          ...
          >
            <Tooltip type='simple'>
              ...
              <Tooltip.Content>
                {connectionStatus ? 'Run your query' : 'Connect to your database first'}
              </Tooltip.Content>
            </Tooltip>
            <Tooltip type='simple'>
              <Tooltip.Trigger hasButtonWrapper>
