...
| image::DesktopChatbotLight.png[DesktopChatbotLight,width=600,height=600]
| image::MobileChatbotLight.png[MobileChatbotLight,width=150,height=150]
|===
...
| image::DesktopChatbotDark.png[DesktopChatbotDark,width=600,height=600]
| image::MobileChatbotDark.png[MobileChatbotDark,width=150,height=150]
|===
...
Then, we will define a new function `fetchResponseFromAPI` that will be responsible for fetching the chatbot's response from the backend based on the user's message (Ideally you will want to use a framework to manage the states, caching and hooks for you like `tanstack/react-query` for example)
